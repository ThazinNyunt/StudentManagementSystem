...
switch ($action) {
  ...
  // `sched_time`, `sched_day`, `sched_semester`, `sched_sy`, `sched_room`, `COURSE_ID`, `SUBJ_ID`
		...
		// AND sched_semester='" .$_POST['sched_semester']."'"; 


			$query = "SELECT *
						FROM tblschedule
						WHERE INST_ID = ".$_POST['INST_ID']."
  						AND sched_day = '".$_POST['sched_day']."'
  						AND STR_TO_DATE(TIME_TO, '%h:%i %p') >= STR_TO_DATE('" .$_POST['TIME_FROM'] . "', '%h:%i %p')";
		...
		$result = mysqli_query($mydb->conn,$query) or die(mysqli_errno($mydb->conn));
 		...
 		if ($numrow > 0) {
 			# code...
 			message("Instructor is not available or is already occupied with the time you have set.","error");
			redirect('index.php?view=add');
 		}else{
